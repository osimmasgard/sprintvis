...
function run() {
    ...
    Promise.all([listOfUsers, listOfDone, listOfSwimLaneID]).then(function (values) {
            ...
            return new Promise(function (res, rej) {
                let bla = values[2].forEach(elem => {
                    if (typeof (elem) != "undefined") {
                        if (elem.ID == id) {
                            //console.log("Found Swimlane", elem.Namn)
                            res(elem.Namn);
                        }
                    }
                })
        ...
        let taskss = Promise.all(values[1].map(x => x.tasks).map(y => y.map(z => {
                return new Promise(function (res, rej) {
                        getSwimlaneName(z.swimlaneId).then(function (elem) {
                            getCollaborators(z.collaborators).then(function (array) {
                                let x = {
                                    "name": z.name,
                                    "collaborators": array,
                                    "swimlaneid": elem
        ...
        taskss.then(function (arr) {
            console.log("ARRRRR", arr[0]);
            Promise.all(arr[0]).then(function (sdf) {
                console.log("elem", sdf);
                $("ol.tasks").append(" <li>" + JSON.stringify(sdf) + "</li>");
            })
        })
    });
};
